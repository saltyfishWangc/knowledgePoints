...
//        byte[] decoded = Base64.decodeBase64(publicKey);
        ...
        byte[] decoded = decoder.decodeBuffer(publicKey);
        RSAPublicKey pubKey = (RSAPublicKey) KeyFactory.getInstance(KEY_ALGORITHM).generatePublic(new X509EncodedKeySpec(decoded));
        //RSA加密
        Cipher cipher = Cipher.getInstance(KEY_ALGORITHM);
        cipher.init(Cipher.ENCRYPT_MODE, pubKey);
//        String outStr = Base64.encodeBase64String(cipher.doFinal(str.getBytes(UTF8)));
//        String outStr = Base64.decodeBase64(cipher.doFinal(str.getBytes(UTF8)));
        BASE64Encoder encoder = new BASE64Encoder();
        String outStr = encoder.encode(cipher.doFinal(str.getBytes(UTF8)));
        return outStr;
    }
    ...
    public static String decrypt(String str, String privateKey) throws Exception {
        ...
        // base64编码的私钥
        byte[] decoded = Base64.decodeBase64(privateKey.getBytes("UTF8"));
